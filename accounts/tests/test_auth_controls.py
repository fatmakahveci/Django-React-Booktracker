...
class AuthRateLimitTests(TestCase):
    ...
    def test_login_limit_cannot_be_bypassed_with_forwarded_headers(self):
        with patch.object(LoginThrottle, 'rate', '2/min', create=True):
            ...
            self.assertEqual(self.client.post('/token/', {}, REMOTE_ADDR='192.0.2.4').status_code, 400)
...
class LogoutTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = CustomUser.objects.create_user('logout@example.com', 'logout_reader', 'Password42!')
    ...
    def test_logout_revokes_only_the_submitted_refresh_token(self):
        ...
        self.assertEqual(self.client.post('/token/refresh/', {'refresh': str(self.token)}).status_code, 401)
        self.assertEqual(self.client.post('/token/refresh/', {'refresh': str(other_session)}).status_code, 200)
    ...
    def test_invalid_or_missing_refresh_cannot_revoke_another_session(self):
        ...
        self.assertEqual(self.client.post('/token/refresh/', {'refresh': str(self.token)}).status_code, 200)

    def test_repeated_logout_keeps_token_revoked(self):
        self.assertEqual(self.client.post('/logout/', {'refresh': str(self.token)}).status_code, 200)
        self.assertEqual(self.client.post('/logout/', {'refresh': str(self.token)}).status_code, 401)
        self.assertEqual(self.client.post('/token/refresh/', {'refresh': str(self.token)}).status_code, 401)
