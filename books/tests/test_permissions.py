...
class BookOwnershipTests(TestCase):
    ...
    def setUpTestData(cls):
        cls.owner = CustomUser.objects.create_user(
            "owner@example.com", "owner", "test-only-password"
        )
        cls.other = CustomUser.objects.create_user(
            "other@example.com", "other", "test-only-password"
        )
        cls.book = Book.objects.create(
            user=cls.other, title="Private", author="Author", year=2025, finished=True
        )
    ...
    def test_lists_never_expose_other_users_books(self):
        for path in ("/books/", "/books/finished/", "/books/unfinished/"):
            with self.subTest(path=path):
                ...
                self.assertEqual(
                    response.data["results"] if path == "/books/" else response.data, []
                )
