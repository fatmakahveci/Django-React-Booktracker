...
# Validate required inputs before stopping services or running migrations.
: "${DJANGO_PUBLIC_URL:?Export DJANGO_PUBLIC_URL}"
...
if [[ "$mode" == deploy ]]; then
  ...
  python3 -c 'import hashlib, pathlib, sys; p=pathlib.Path(sys.argv[1]); f=p.open("rb"); digest=hashlib.file_digest(f,"sha256").hexdigest(); f.close(); pathlib.Path(str(p)+".sha256").write_text(digest+"\n")' "$backup"
...
curl --fail --silent --show-error --connect-timeout 10 --max-time 30 "${DJANGO_PUBLIC_URL%/}/api/health/ready/"
